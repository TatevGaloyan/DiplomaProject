...
def create_call(request):
    url = 'https://212.34.248.234:9443/clicktocall.cgi'
    #caller = '711007'
    user = request.POST.get('extusername')
    password = request.POST.get('extpassword')
    ext_id = 'Chrome_extension 11'
    app = 'Chrome extension'

    data = Data.objects.get(username=request.user.username)
    data.extusername = user
    data.extpassword = password
    data.save()
